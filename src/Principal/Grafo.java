...
public class Grafo {
    ...
    public List<String> buscaRelacionados(String vertice) {/*
         ...
         */
        if (grafo.containsKey(vertice.toUpperCase()))/*Busca pela chave correspondente dentro do grafo, transformando-a em maiuscula para coincidir com o dado cadastrado*/ {
            return grafo.get(vertice.toUpperCase());//Retorna o valor encontrado

        }
        return null;//Retorna uma lista nula caso não haja um dado correspondente
    }

    /*Como parâmetro recebe um ArrayList<String>, e solicita ao usuário os dados que deverão ser relacionados*/
    public void automatizaExemplos(ArrayList<String> listaExemplos, String familiaExemplos) {
        this.adcVertice(familiaExemplos.toUpperCase());
        for (String item : listaExemplos) {
    ...
    public void exibeTodos() {
        ...
        Map<String, List<String>> gafoDeTodos = this.getGrafo(); // Map de apoio para armazenar as informações
        StringBuilder stringBuilder = new StringBuilder(); //construtor de string
        for (Map.Entry<String, List<String>> entry : gafoDeTodos.entrySet()) {/*Laço de repetição para iterar sobre o conjunto de chaves e valores*/
            String chave = entry.getKey();//armazena as chaves
            List<String> valores = entry.getValue();//armazena os valores das chaves
            stringBuilder.append(chave).append(":\n").append(valores).append("\n\n");//grava no string builder, conforme a formatação aplicada, as chaves e valores
        }
        ...
        frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);//libera os recursos alocados pela tela após seu fechamento
        ...
        JScrollPane scrollPane = new JScrollPane(textArea);//cria a janela rolavel com a textArea contendo os resultados
